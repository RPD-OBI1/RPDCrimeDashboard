...
sectionmap <- leaflet() %>% 
    ...
    fitBounds(lng1 = -77.575370, lat1 = 43.159093, lng2 = -77.626865, lat2 = 43.257633)
...
MCUhoms <- mculist[[1]] %>%
    ...
    mutate(CrimeType = "Homicide",
           DateOfEvent = lubridate::as_date(DateOfEvent),
           YearofEvent = lubridate::year(DateOfEvent),
           DOYofEvent = lubridate::yday(DateOfEvent)) %>%
...
if(nrow(MCUhoms) > 0){
    MCUhoms$SevenDaysGroup <- sevenDayGroupTable$Group[match(MCUhoms$DOYofEvent, 
                                                             sevenDayGroupTable$DOY)]
...
cfs.df <- LERMS_CFS_query(begindate = begindate, section = sectionname) %>%
...
```{r}
PartOne <- c("Aggravated Assault", "Arson", "Burglary", "Homicide", "Larceny",
                 "MV Theft", "Rape", "Robbery")

treedata <- df %>%
    filter(CrimeType %in% PartOne |
               Firearm == "Firearm") %>%
...
map <- hotspotMap(sectionmap, 
                  filter(guns, 
                         Lat != -361 & SevenDaysGroup > curr.week - 4))
map
...
if(nrow(personincident) > 0){
    ...
    DT::datatable(persons,
                  ...
                  options = list(pageLength = 100,
                                 dom = 'tf',
                                 columnDefs = list(list(className = 'dt-right', targets = 0:9))))
    }
...
rapes %>%
    ...
    DT::datatable(filter = 'top',
                  rownames = FALSE,
                  colnames = c('Case Number', 'Address', 'Event Date', 
                               'Report Date', 'Location', 'Clearance'),
                  options = list(pageLength = 100,
                                 dom = 'tf',
...
The Office of Business Intelligence has calculated "action limits" which you can see represented as dashed lines on the chart on the right hand side of this page. These limits (both upper and lower) are calculated by using weighted 3 year averages as centering values, and then calculating the standard deviations to find how much variation from that average to expect.
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceTreeMap.png')
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceProjectionPlot.png')
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceAoristicPlot.png')
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceMapClinton.png')

```


### CFS - Non-discretionary plot
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceNonDiscretionaryCFSPlot.png')

```


### CFS - Discretionary plot
...
include_graphics('Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceDiscretionaryCFSPlot.png')

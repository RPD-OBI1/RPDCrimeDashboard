...
### Projection for Shooting Victims
...
### Projection for Aggravated Assault
...
if(nrow(personincident) > 0){
    persons <- bind_rows(personincident %>% 
                         ...
                         select(CaseNumber, DateOfEvent, DateOfDeath, Name, 
                                PersonType, CaseStatus, Beat, WeaponType),
                         personincident %>% 
    ...
    DT::datatable(persons,
                  filter = 'top',
                  rownames = FALSE,
                  colnames = c('Case Number', 'Event Date', 'Date of Death', 'Name',
                               'Person Type', ' Case Status', 'Beat', 'Weapon Type'),
                  options = list(pageLength = 100,
                                 dom = 'tf',
...
### Wait time (RPD arrival time) for non-discretionary calls since `r begindate`
...
### On scene time for discretionary calls since `r begindate`
...
![](Y:/Projects/dashboard/RPDCrimeDashboard/Objects/ReferenceMapLake.png)
